...
freqRange = [0.13 4]; % Range of frequencies to be displayed in XWT
phaseType = 'all'; % 'all' - displays all phases;
% 'alt' - displays alternating phases only
% 'synch' - displays synchronous phases only

traceType = 'smooth'; % 'raw'    - displays signals as raw traces
...
motherWavelet = 'Morlet'; %%%('Morlet', 'Paul','DOG')
...
wb = waitbar(50/600,'Initializing');
...
clear sigxy
waitbar(153/600,wb,'Computing XW')
...
for fileNum = 1:size(sigMat,2) % File Number Loop # 1
    ...
    for chNum =1:size(sigMat,3)-1 % Channel Number Loop # 1
       ...
       if isempty(Wxy_nonzero_lin)
        ...
        %% PLOTTING FIGURES
        waitbar(341/600,wb,'Generating Figures')
       ...
       switch plotfig
        ...
        %% AVERAGED XW PLOTS
waitbar(535/600,wb,'Averaging XWs')
...
waitbar(547/600,wb,'Storing Data Into An Excel Sheet')
...
%%%%   Average XW Plots   %%%%%%
waitbar(588/600,wb,'Plotting Extra Stuff')
Wxy_avg  = xwplotavg(Wxy3d.(chStr),sigxy.(chStr),time_reduced, period, coi);
avgCheck = 1;
[mfvec,pfvec] = instantaneouswavefreq(Wxy_avg,freq);
tvpower = instantaneouswavepow(Wxy_avg);
dynamicfreqpowplot
plotphase

waitbar(1,wb,'Congratulations!')
pause(1);
close(wb)
