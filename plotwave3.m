function plotwave3(Wxy, time, period, coi, sig95, sigmax, sigmay)
...
ordinateType = 'log'; % 'linear' or 'log' %%%% Why does changing this to linear affect the colorscale?
...
%      Wxy_max_normalized = Wxy_variance_normalized/max(Wxy_variance_normalized(:));   
      H= contourf(time,freq,Wxy_variance_normalized); shading flat
...
if strcmpi('ordinateType','linear')
    ...
    if find(abs(Wxy)==0)
        [c,h] = contour(time,log2(freq),abs(Wxy),[0 0],'w');
        set(h,'linewidth',2)
    else
        [c,h] = contour(time,log2(freq),sig95,[1 1],'w');
        set(h,'linewidth',2)
    end
end
...
if strcmpi(ordinateType,'linear')
    ...
    if strcmpi(coiType,'fill')
        ...
        set(hcoi,'alphadatamapping','direct','facealpha',0.5)
    else
